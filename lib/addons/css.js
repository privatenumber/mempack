...
const importAlias = postcss.plugin('postcss-import-alias', (alias) => function (css) {
	css.walkAtRules('import', (rule) => {
		const resolved = Object.entries(alias).reduce((req, [key, val]) => req.replace(new RegExp(`^${key}`), val), parseQuoted(rule.params));
